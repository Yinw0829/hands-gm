app.controller('appRecruit', ['$scope', '$modal', '$log', '$http', '$resource', 'api', '$state','Storage',function ($scope, $modal, $log, $http, $resource, api, $state,Storage) {
    var cityKey = 'city';
    ...
    // 热门招聘
    getSta.MONTH1({type:'publishCount'},function (data) {
        $scope.MonthLogin = data;
    });
    getSta.WEEK1({type:'publishCount'},function (data) {
        $scope.WeekLogin = data;
    });
    getSta.DAY1({type:'publishCount'},function (data) {
        $scope.DayLogin = data;
    });
    // 热门求职
    getSta.MONTH2({type:'applyCount'},function (data) {
        $scope.MonthAttes = data;
    });
    getSta.WEEK2({type:'applyCount'},function (data) {
        $scope.WeekAttes = data;
    });
    getSta.DAY2({type:'applyCount'},function (data) {
        $scope.DayAttes = data;
    });
